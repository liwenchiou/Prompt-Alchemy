...
test.describe("後台管理與 CRUD 功能 (Admin Operations) 端到端測試 - Real DB 連動", () => {
  ...
  test("後台 Prompt / Skill 列表正確載入 Real DB 資料", async ({ page }) => {
    await expect(page.locator("text=Prompt / Skill 管理").first()).toBeVisible();

    // 驗證 Real DB 預設種子資料呈現於表格中 (例如 後端 API 審查)
  ...
  test("點擊『+ 新增 Prompt / Skill』可順利進入新增表單頁面", async ({ page }) => {
